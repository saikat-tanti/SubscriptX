  ...
  return (
    <div className="min-h-screen bg-[#030712] py-8 px-4 sm:px-6 lg:px-8 mx-auto max-w-7xl space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Total Plans */}
        <Card className="relative overflow-hidden border-slate-800 bg-slate-900/60 backdrop-blur-xl">
          ...
          <CardContent>
            <div className="text-3xl font-extrabold text-white font-mono">{totalPlans}</div>
            <p className="text-xs text-slate-400 mt-1">Published Subscription Contracts</p>
          </CardContent>
        </Card>
        ...
        <Card className="relative overflow-hidden border-slate-800 bg-slate-900/60 backdrop-blur-xl">
          ...
          <CardContent>
            <div className="text-3xl font-extrabold text-purple-400 font-mono">{activeSubCount}</div>
            <p className="text-xs text-slate-400 mt-1">Active Wallet Passes</p>
          </CardContent>
        </Card>
        ...
        <Card className="relative overflow-hidden border-slate-800 bg-slate-900/60 backdrop-blur-xl">
          ...
          <CardContent>
            <div className="text-3xl font-extrabold text-emerald-400 font-mono">{totalTxCount}</div>
            <p className="text-xs text-slate-400 mt-1">On-Chain Interaction History</p>
          </CardContent>
        </Card>
        ...
        <Card className="relative overflow-hidden border-slate-800 bg-slate-900/60 backdrop-blur-xl">
          ...
          <CardContent>
            <div className="text-3xl font-extrabold text-white font-mono">
              {formatXlm(balanceXlm)} <span className="text-xs font-normal text-amber-400">XLM</span>
            </div>
            <p className="text-xs text-slate-400 mt-1">Stellar Testnet Account</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <Card glow className="lg:col-span-1 flex flex-col justify-between">
            ...
            <div className="py-6 space-y-4">
              <div>
                ...
                <p className="text-4xl font-extrabold text-white font-mono mt-1">
                  {formatXlm(merchantRevenue)} <span className="text-base text-purple-400 font-sans">XLM</span>
                </p>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4 space-y-2 text-xs text-slate-400">
                <div className="flex justify-between">
                  <span>Protocol Fee Split:</span>
                  <span className="font-mono text-slate-200">1.5% (150 BPS)</span>
                </div>
                <div className="flex justify-between">
                  <span>Contract Security:</span>
                  <span className="text-emerald-400 font-semibold">Soroban Guarded</span>
                </div>
                <div className="flex justify-between">
                  <span>Withdrawal Mode:</span>
                  <span className="font-mono text-slate-200">Instant Self-Custody</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <Button
            ...
            variant="primary"
            className="w-full gap-2 shadow-purple-600/20"
          >
          ...
          <CardContent className="pt-4">
            {transactions.length === 0 ? (
              <div className="text-center py-12 text-slate-500 space-y-2">
                <Activity className="h-8 w-8 mx-auto text-slate-600 animate-pulse" />
              ...
              <div className="space-y-3">
                {transactions.slice(0, 5).map((tx, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between rounded-xl border border-slate-800/80 bg-slate-950/60 p-3.5 hover:border-slate-700 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-800 text-blue-400 font-mono text-xs font-bold">
